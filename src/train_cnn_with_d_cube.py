...
def main():
    ...
    if all(config.get(k) for k in ['val_s2_bands_file_path', 'val_s2_masks_file_path',
        ...
        from datasets.ssl_dataset import AustrianCropValidationPatch, ThreadSafeDataLoader
        val_dataset = AustrianCropValidationPatch(
            ...
            max_cloud_ratio=0.8,
            standardize=True,
            cache_dir="data/ssl_training/austrian_crop/cache",  # 启用缓存
            n_workers=16,               # 使用16个工作进程
            precompute=True             # 预计算所有样本
        )
        # val_loader = DataLoader(val_dataset, batch_size=512, shuffle=False, num_workers=0)
        val_loader = ThreadSafeDataLoader(
            val_dataset,
            batch_size=256,
            shuffle=True,
            num_threads=16  # 使用16个线程
        )
        logging.info(f"Validation dataset loaded with {len(val_loader.dataset)} samples.")
