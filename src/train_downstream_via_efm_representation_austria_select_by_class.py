...
def evaluate_model(model, data_loader, device, num_classes, class_names=None):
    ...
    # 计算平衡准确率（宏平均召回率）
    balanced_acc = recall_score(all_targets, all_preds, average='macro')
    
    # 计算每个类别的性能指标
    class_report = classification_report(all_targets, all_preds, output_dict=True, digits=4)
    ...
    # 添加每个类别的指标
    for i in range(1, num_classes + 1):  # 类别从1开始
        if str(i-1) in class_report:  # 在报告中索引从0开始
            cls_metrics = class_report[str(i-1)]
            metrics[f'class_{i}_precision'] = cls_metrics['precision']
            metrics[f'class_{i}_recall'] = cls_metrics['recall']
            metrics[f'class_{i}_f1'] = cls_metrics['f1-score']
    
    # 生成易读的分类报告
    if class_names:
        # 此处需要调整索引，使其与分类报告中的索引匹配
        # 我们的模型标签从0开始，但实际上映射到了类别1-17
        target_names = class_names
        report_str = classification_report(all_targets, all_preds, target_names=target_names, digits=4)
    else:
        report_str = classification_report(all_targets, all_preds, digits=4)
...
def main():
    # 解析命令行参数
    # 使用方式：/mnt/d/wsl_anaconda3/bin/python src/train_downstream_via_representation_austria_select_by_class.py --sample_per_pixel 10 --num_experiment 200
    parser = argparse.ArgumentParser(description='Land Classification with Multiple Experiments')
    parser.add_argument('--sample_per_pixel', type=int, default=20, help='Number of samples per class')
    parser.add_argument('--num_experiment', type=int, default=100, help='Number of experiments to run')
    ...
    parser.add_argument('--representation_path', type=str, 
                        # default="data/representation/austrian_crop_whole_year_downsample_100_fsdp_20250427_084307.npy", 
                        # default="data/representation/austrian_crop_downsample_100_fsdp_20250407_195912.npy", 
                        default="data/representation/austrian_crop_EFM_Embeddings_2022_downsample_100.npy",
    ...
    checkpoint_name = f"{representation_file_name}_{timestamp}"
