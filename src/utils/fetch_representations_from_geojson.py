#!/usr/bin/env python3
"""
Grid Data Downloader with GeoJSON Intersection - Optimized Version
Downloads grid data from remote server based on GeoJSON intersection using rsync
...
class OptimizedGridDownloader:
    """Optimized class for downloading grid data based on GeoJSON intersection"""
    ...
    def load_geojson(self, geojson_path):
        """
        Load GeoJSON and ensure it's in WGS84
        ...
        logger.info(f"GeoJSON bounds: Lon [{bounds[0]:.4f}, {bounds[2]:.4f}], Lat [{bounds[1]:.4f}, {bounds[3]:.4f}]")
    ...
    def find_intersecting_grids(self, geojson_gdf, bounds):
        """
        Find all grid cells that intersect with the GeoJSON
        
        Args:
            geojson_gdf: GeoJSON GeoDataFrame
        ...
        for center_lon, center_lat in tqdm(potential_grids, desc="Checking grid intersections"):
            ...
            # Check intersection with GeoJSON
            if geojson_gdf.intersects(grid_box).any():
        ...
        logger.info(f"Found {len(intersecting_grids)} grids intersecting with GeoJSON")
...
def main():
    parser = argparse.ArgumentParser(
        description='Download grid data based on GeoJSON intersection (Optimized)',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Example usage:
    python grid_downloader_geojson.py input.geojson -o /path/to/output -y 2023 2024
    python grid_downloader_geojson.py input.geojson -o /path/to/output -y 2019 2020 2021 2022 2023 2024
    
    /maps/zf281/miniconda3/envs/detectree-env/bin/python \
    /maps/zf281/btfm4rs/src/utils/fetch_representations_from_geojson.py \
    /scratch/zf281/geotessera/example/CB.geojson \
    -y 2017 \
    -o /scratch/zf281/btfm_representation/cambridge
    """
    )
    
    parser.add_argument('geojson', help='Input GeoJSON file path')
    ...
    parser.add_argument('--buffer', type=float, default=0.1,
                        help='Buffer in degrees around GeoJSON bounds (default: 0.1)')
    ...
    try:
        # Load GeoJSON
        geojson_gdf, bounds = downloader.load_geojson(args.geojson)
