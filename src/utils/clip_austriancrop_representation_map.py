...
import argparse
import random
# Added for resizing functionality if representation and label shapes differ.
...
def main(args):
    ...
    repr_path = "/scratch/zf281/btfm_representation/austrian_crop/austrian_crop_efm_v1.0.npy"
    # repr_path = "/scratch/zf281/btfm_representation/austrian_crop/mpc_pipeline_fsdp_20250604_100313.npy"
    # repr_path = "/scratch/zf281/btfm_representation/austrian_crop/austria_Presto_embeddings.npy"
    label_path = "/maps/zf281/btfm-training-10.4/maddy_code/data_processed/fieldtype_17classes.npy"
    ...
    # --- Resize representation to match label shape if they differ ---
    H_repr, W_repr, C_dim = representation.shape
    H_label, W_label = fieldtype.shape

    if H_repr != H_label or W_repr != W_label:
        print(f"Representation shape ({H_repr}, {W_repr}) does not match label shape ({H_label}, {W_label}).")
        print("Resizing representation to match label dimensions...")
        
        # Using scikit-image's resize function for robust resizing of multi-channel data.
        # preserve_range=True keeps the original data value range.
        # anti_aliasing=True is recommended for down-sampling to avoid artifacts.
        representation = resize(
            representation, 
            (H_label, W_label), 
            preserve_range=True, 
            anti_aliasing=True
        ).astype(representation.dtype) # Cast back to the original data type
        
        print(f"Resizing complete. New representation shape: {representation.shape}")

    # Update dimensions for patch extraction
    H, W, C_dim = representation.shape
    
    if fieldtype.shape[0] != H or fieldtype.shape[1] != W:
        # This check now serves as a post-resize validation. It should not be triggered.
        raise ValueError("CRITICAL: Representation and fieldtype dimensions still do not match after resizing!")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_dir", type=str, default="data/downstream/austrian_crop_patch_split_efm", 
