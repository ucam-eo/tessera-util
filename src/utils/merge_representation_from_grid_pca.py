...
def apply_pca_visualization(data_f32, n_components=3, sample_ratio=1.0):
    """
    Apply PCA to multi-dimensional data for visualization.
    
    Args:
        data_f32: float32 ndarray of shape (H, W, C) where C is the number of bands
        n_components: Number of PCA components (default 3 for RGB)
        sample_ratio: Ratio of pixels to sample for PCA fitting (1.0 = all pixels)
    
    Returns:
        pca_data: PCA-transformed data of shape (H, W, n_components)
    """
    ...
    data_reshaped = data_f32.reshape(-1, c)
    
    # Find valid pixels (non-zero pixels)
    valid_mask = np.any(data_reshaped != 0, axis=1)
    valid_pixels = data_reshaped[valid_mask]
    
    if len(valid_pixels) == 0:
        print("No valid pixels found for PCA")
        return np.zeros((h, w, n_components), dtype=np.float32)
    
    print(f"Found {len(valid_pixels)} valid pixels out of {len(data_reshaped)} total pixels")
    
    # Sample pixels for PCA fitting if requested
    if sample_ratio < 1.0:
        n_sample = int(len(valid_pixels) * sample_ratio)
        if n_sample > 0:
            sample_indices = np.random.choice(len(valid_pixels), size=n_sample, replace=False)
            sample_pixels = valid_pixels[sample_indices]
        else:
            sample_pixels = valid_pixels
        print(f"Using {len(sample_pixels)} pixels for PCA fitting")
    else:
        sample_pixels = valid_pixels
        print("Using all valid pixels for PCA fitting")
    ...
    # Transform all data
    pca_transformed = np.zeros((len(data_reshaped), n_components), dtype=np.float32)
    pca_transformed[valid_mask] = pca.transform(valid_pixels)
    
    # Reshape back to spatial dimensions
    pca_data = pca_transformed.reshape(h, w, n_components)
...
def main(base_dir=None, downsample_factor=1, rgb_only=False, filter_cross_dateline_tiles=True, 
         force_projected=True, output_dir=None, output_repr_format='tiff', use_pca_visualization=False):
    """
    ...
    Args:
        ...
        use_pca_visualization: If True, use PCA on all 128 bands for RGB visualization instead of first 3 bands
    """
    # Set paths
    if base_dir is None:
    ...
    try:
        ...
        # For PCA visualization, we need all bands regardless of rgb_only setting
        process_all_bands = use_pca_visualization or not rgb_only
        bands_str = "all 128 bands" if process_all_bands else "RGB (3 bands)"
        ...
        with ProcessPoolExecutor(max_workers=num_workers) as executor:
            future_to_grid = {
                executor.submit(create_multiband_data, str(folder), temp_dir, 
                              ...
                              rgb_only=(not process_all_bands)): folder 
        ...
        # Output filename
        bands_suffix = "rgb" if rgb_only and not use_pca_visualization else f"{num_bands}bands"
        ...
        success = merge_multiband_optimized(tiff_info, resolution, output_path, scales_output_path, target_crs,
                                          rgb_only=(rgb_only and not use_pca_visualization), 
        ...
        # Step 3: Create RGB visualization with dequantization and optional PCA
        print("\nStep 3: Creating RGB visualization...")
        if use_pca_visualization:
            print("Using PCA visualization with all bands")
        ...
        if output_repr_format == 'tiff':
            with rasterio.open(output_path) as src:
                # Read all bands as int8 for PCA, or just RGB for normal viz
                if use_pca_visualization:
                    # Read all bands
                    all_bands_int8 = np.zeros((src.height, src.width, num_bands), dtype=np.int8)
                    for i in range(num_bands):
                        all_bands_int8[:, :, i] = src.read(i + 1)
                    data_for_viz = all_bands_int8
                else:
                    # Read only RGB bands
                    rgb_int8 = np.zeros((src.height, src.width, 3), dtype=np.int8)
                    for i in range(min(3, num_bands)):
                        rgb_int8[:, :, i] = src.read(i + 1)
                    data_for_viz = rgb_int8
            ...
            if use_pca_visualization:
                ...
                data_for_viz = data[:, :, :3].copy()
            crs = target_crs
        
        # Dequantize if scales are available
        if has_scales_for_viz:
            print("Dequantizing bands using scales...")
            scales_data = np.load(scales_output_path, mmap_mode='r')
            
            # Dequantize all bands or just RGB
            data_f32 = load_and_dequantize_representation(data_for_viz, scales_data)
            print("Dequantization complete.")
        else:
            # No scales available, convert int8 to float32 directly
            data_f32 = data_for_viz.astype(np.float32)
        
        # Apply PCA if requested
        if use_pca_visualization:
            print("Applying PCA for visualization...")
            pca_data = apply_pca_visualization(data_f32, n_components=3, sample_ratio=1.0)
            
            # Convert PCA results to RGB
            ...
            # Standard RGB visualization (first 3 bands)
            rgb_array = np.zeros((height, width, 3), dtype=np.uint8)
            for i in range(min(3, data_f32.shape[2])):
                if has_scales_for_viz:
                    band_data = data_f32[:, :, i]
                    # Normalize to 0-255 range
                    valid_mask = np.isfinite(band_data) & (band_data != 0)
                    if np.any(valid_mask):
                        band_min = np.min(band_data[valid_mask])
                    ...
                    rgb_array[:, :, i] = ((data_for_viz[:, :, i].astype(np.float32) + 127.0) * (255.0 / 254.0)).astype(np.uint8)
        ...
        # Save image
        viz_suffix = "_pca" if use_pca_visualization else ("_dequantized" if has_scales_for_viz else "")
        ...
        if use_pca_visualization:
            print(f"  - Visualization: PCA RGB from all {num_bands} bands")
        else:
            print(f"  - Visualization: {'Dequantized RGB' if has_scales_for_viz else 'Raw int8 converted to uint8'}")
...
if __name__ == "__main__":
    ...
    main(
        base_dir="/scratch/zf281/btfm_representation/cairngorms/2023", 
        output_dir="/scratch/zf281/btfm_representation/cairngorms",
        ...
        use_pca_visualization=True,  # NEW: If True, use PCA on all 128 bands for RGB visualization
    )