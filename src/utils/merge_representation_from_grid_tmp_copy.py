...
def main(base_dir=None, year=None, downsample_factor=1, rgb_only=False, filter_cross_dateline_tiles=True, 
    ...
    Args:
        ...
        year: Year to add to the filename prefix
    ...
    try:
        ...
        # Output filename with year prefix
        ...
        # Add year prefix to output files
        year_prefix = f"{year}_" if year else ""
        output_path = os.path.join(output_dir, f"{year_prefix}{location_name}_map_{resolution}m_{crs_name}_{bands_suffix}{filter_suffix}{output_ext}")
        scales_output_path = os.path.join(output_dir, f"{year_prefix}{location_name}_map_{resolution}m_{crs_name}_scales{filter_suffix}.npy")
        ...
        # Save image with year prefix
        output_png = os.path.join(output_dir, f"{year_prefix}{location_name}_map_{resolution}m_{crs_name}_{bands_suffix}{filter_suffix}.png")
...
if __name__ == "__main__":
    # Loop through years 2017-2024
    base_location = "/maps/zf281/btfm4rs/data/downstream/pv_detection/roi_1"
    output_location = "/maps/zf281/btfm4rs/data/downstream/pv_detection/roi_1"
    ...
    for year in range(2017, 2025):
        ...
        main(
            base_dir=f"{base_location}/{year}", 
            year=year,  # Pass the year to add to filenames
