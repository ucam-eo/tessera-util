...
def create_rsync_script(txt_file, source_base, target_host, target_user, target_path, target_path2, script_file):
    """
    创建rsync脚本用于传输文件到两个目标路径
    如果target_path或target_path2为空字符串，则跳过对应的传输
    """
    ...
    with open(script_file, 'w') as f:
        ...
        f.write(f"# Number of targets: {targets_count}\n\n")
        
        # 第一个目标路径
        if target_path:
            f.write("# === Target 1 ===\n")
            f.write(f"# Target path: {target_path}\n")
            f.write("echo '=== Transferring to Target 1 ==='\n")
            f.write(f"echo 'Target path: {target_path}'\n\n")
            
            f.write("# Create target directory if not exists\n")
            f.write(f"ssh {target_user}@{target_host} 'mkdir -p {target_path}'\n\n")
            
            f.write("# Transfer files to target 1\n")
            f.write(f"cd {source_base}\n")
            f.write(f"rsync -avzP --files-from={os.path.abspath(txt_file)} . {target_user}@{target_host}:{target_path}/\n")
            f.write("echo 'Completed transfer to Target 1'\n\n")
        else:
            f.write("# Target 1 is empty - skipping\n\n")
        
        # 第二个目标路径
        if target_path2:
            f.write("# === Target 2 ===\n")
            f.write(f"# Target path: {target_path2}\n")
            f.write("echo '=== Transferring to Target 2 ==='\n")
            f.write(f"echo 'Target path: {target_path2}'\n\n")
            
            f.write("# Create target directory if not exists\n")
            f.write(f"ssh {target_user}@{target_host} 'mkdir -p {target_path2}'\n\n")
            
            f.write("# Transfer files to target 2\n")
            f.write(f"cd {source_base}\n")
            f.write(f"rsync -avzP --files-from={os.path.abspath(txt_file)} . {target_user}@{target_host}:{target_path2}/\n")
            f.write("echo 'Completed transfer to Target 2'\n\n")
        else:
            f.write("# Target 2 is empty - skipping\n\n")
        
        # 完成信息
        f.write("echo '=== All transfers completed ==='\n")
        
        # 如果没有有效的目标路径
        if not target_path and not target_path2:
            f.write("\necho 'WARNING: No valid target paths provided - no files will be transferred!'\n")
    ...
    # 显示目标路径信息
    if target_path and target_path2:
        print(f"将传输文件到两个目标路径:")
        print(f"  1. {target_path}")
        print(f"  2. {target_path2}")
    elif target_path:
        print(f"将传输文件到目标路径: {target_path}")
    elif target_path2:
        print(f"将传输文件到目标路径: {target_path2}")
    else:
        print("警告: 没有有效的目标路径，不会传输任何文件!")
...
def main():
    ...
    boundary_path = "/maps/zf281/btfm4rs/cci_workshop_roi_shp/Kenya_points/training_kenya_shapefile.shp"  # 修改为您的SHP文件路径
    
    # 输出文件
    region_name = "external_request_#68_2"  # 可以修改为具体地区名称，如"中国"、"美国"等
    ...
    # rsync参数
    target_host = "otrera.cl.cam.ac.uk"  # 目标服务器
    target_user = "zf281"  # 目标用户名
    
    # 两个目标路径 - 如果设置为空字符串""，则不会传输到该路径
    target_path = f"/tank/{target_user}/global_0.1_degree_tiff_express"  # 第一个目标路径
    target_path2 = f"/tank/{target_user}/global_0.1_degree_tiff"  # 第二个目标路径
    
    # 如果不想传输到某个路径，可以设置为空字符串，例如:
    # target_path = ""  # 这样就不会传输到第一个路径
    # target_path2 = ""  # 这样就不会传输到第二个路径
    # =============================
    ...
    # 创建rsync脚本 - 现在支持两个目标路径
    create_rsync_script(
        output_txt, tiff_dir, target_host, target_user, target_path, target_path2, rsync_script
    )
