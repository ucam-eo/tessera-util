...
class AustrianCropValDataset(Dataset):
    ...
    def _process_s2(self, bands, masks, doys):
        ...
        for t in range(len(doys)):
            ...
            pos = day - 1  # 转为0-index
            if masks[t] == 1:
                new_mask[pos] = 2  # 有值且有效
            else:
                new_mask[pos] = 1  # 有值但无效(云遮挡)
...
class TimeSeriesTransformer(nn.Module):
    ...
    def __init__(self, input_dim, embed_dim, output_dim, nhead=8, num_layers=4, dropout=0.1):
        ...
        # 缺失标志矩阵 - 用于标记缺失数据的时间步
        self.missing_flag_matrix = nn.Parameter(torch.randn(embed_dim, embed_dim))
    ...
    def forward(self, x, mask):
        ...
        # 创建缺失掩码(mask=0的位置)
        missing_mask = (mask == 0).unsqueeze(-1).expand(-1, -1, self.embed_dim) # (batch_size, seq_len, embed_dim)
        ...
        # 应用云标志矩阵
        cloud_transformed = torch.mm(reshaped_embedded, self.cloud_flag_matrix)  # (batch_size*seq_len, embed_dim)
        cloud_transformed = cloud_transformed.reshape(batch_size, seq_len, self.embed_dim)  # (batch_size, seq_len, embed_dim)
        
        # 应用缺失标志矩阵
        missing_transformed = torch.mm(reshaped_embedded, self.missing_flag_matrix)  # (batch_size*seq_len, embed_dim)
        missing_transformed = missing_transformed.reshape(batch_size, seq_len, self.embed_dim)  # (batch_size, seq_len, embed_dim)
        
        # 只在mask == 1(云遮挡)的位置应用云变换
        embedded = torch.where(cloud_mask, cloud_transformed, embedded)
        
        # 只在mask == 0(缺失)的位置应用缺失变换
        embedded = torch.where(missing_mask, missing_transformed, embedded)
        
        # 添加位置编码
        embedded = self.pos_encoder(embedded)
        
        # 应用Transformer编码器 - 不再使用src_key_padding_mask
        encoded = self.transformer_encoder(embedded)
        # 输出形状: (batch_size, seq_len, embed_dim)
        
        # 应用时间注意力池化
        pooled = self.temporal_pool(encoded, mask != 0)  # 只有非零掩码值参与池化
