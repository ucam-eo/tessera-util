...
def parse_args():
    parser = argparse.ArgumentParser(description="Multi-tile Inference on CPU")
    parser.add_argument('--config', type=str, default="configs/multi_tile_infer_config_cpu.py", 
                        help="Path to config file")
    parser.add_argument('--tile_path', type=str, required=True, 
                        help="Path to the tile to process")
    parser.add_argument('--process_id', type=int, default=0, 
                        help="Process ID for logging purposes")
    parser.add_argument('--num_threads', type=int, default=1,
                        help="Number of CPU threads to use for this process")
...
def main():
    ...
    log_file = f"logs/infer_process_{args.process_id}.log"
    ...
    try:
        # Set the number of threads for this process
        torch.set_num_threads(args.num_threads)
        logging.info(f"[Process {args.process_id}] Set PyTorch to use {args.num_threads} threads")
        
        # Load configuration
        config_module = load_config_module(args.config)
        config = config_module.config
        
        # Setup device (CPU for all processes)
        device = torch.device("cpu")
        logging.info(f"[Process {args.process_id}] Using device: CPU")
        ...
        logging.info(f"[Process {args.process_id}] Will process tile: {tile_name}")
        ...
        logging.info(f"[Process {args.process_id}] Processing tile: {tile_name}")
        try:
            process_tile(tile_path, output_path, model, config, device, args.process_id)
            logging.info(f"[Process {args.process_id}] Completed tile: {tile_name}")
        except Exception as e:
            logging.error(f"[Process {args.process_id}] Error processing tile {tile_name}: {str(e)}")
            logging.error(traceback.format_exc())
            sys.exit(1)
        
        logging.info(f"[Process {args.process_id}] Tile processed successfully")
