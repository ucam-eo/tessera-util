...
from datasets.downstream_dataset import AustrianCropInference_64_Fixed, austrian_crop_64_fixed_collate_fn
from models.modules import TransformerEncoder_64_Fixed, ProjectionHead
from models.downstream_model import ClassificationHead, MultimodalDownstreamModel64Fixed
...
def parse_args():
    ...
    parser.add_argument('--config', type=str, default="configs/downstream_config_64_fixed.py", help="Path to config file (e.g. configs/downstream_config.py)")
...
def main():
    ...
    train_dataset = AustrianCropInference_64_Fixed(
    ...
    val_dataset = AustrianCropInference_64_Fixed(
        s2_bands_file_path=config['s2_bands_file_path'],
        s2_masks_file_path=config['s2_masks_file_path'],
        s2_doy_file_path=config['s2_doy_file_path'],
        s1_asc_bands_file_path=config['s1_asc_bands_file_path'],
    ...
    test_dataset = AustrianCropInference_64_Fixed(
        s2_bands_file_path=config['s2_bands_file_path'],
        s2_masks_file_path=config['s2_masks_file_path'],
        s2_doy_file_path=config['s2_doy_file_path'],
        s1_asc_bands_file_path=config['s1_asc_bands_file_path'],
    ...
    train_loader = DataLoader(train_dataset, batch_size=config['batch_size'], shuffle=True,
                              num_workers=config['num_workers'], collate_fn=austrian_crop_64_fixed_collate_fn, pin_memory=True, drop_last=False)
    val_loader = DataLoader(val_dataset, batch_size=config['batch_size'], shuffle=False,
                            num_workers=config['num_workers'], collate_fn=austrian_crop_64_fixed_collate_fn)
    ...
    from models.ssl_model import MultimodalBTModel_64_Fixed
    ssl_model = MultimodalBTModel_64_Fixed(
        s2_enc, s1_enc, projector_ssl, 
    ...
    # )
    logging.info(ssl_model.s2_backbone.fc_out[1].weight)
    ...
    downstream_model = MultimodalDownstreamModel64Fixed(s2_backbone=ssl_model.s2_backbone,
    ...
    logging.info(downstream_model.s2_backbone.fc_out[1].weight)
    ...
    for epoch in range(config['epochs']):
        ...
        train_loss_sum = 0.0
        for s2_batch, s1_batch, s2_mask_batch, s1_mask_batch, labels_batch in tqdm.tqdm(train_loader, desc=f"Epoch {epoch+1}/{config['epochs']} Train"):
            s2_batch = s2_batch.to(device)
            s1_batch = s1_batch.to(device)
            s2_mask_batch = s2_mask_batch.to(device)
            s1_mask_batch = s1_mask_batch.to(device)
            labels_batch = labels_batch.to(device)
            optimizer.zero_grad()
            logits = downstream_model(s2_batch, s1_batch, s2_mask_batch, s1_mask_batch)
        ...
        with torch.no_grad():
            for s2_batch, s1_batch, s2_mask_batch, s1_mask_batch, labels_batch in tqdm.tqdm(val_loader, desc="Validation"):
                s2_batch = s2_batch.to(device)
                s1_batch = s1_batch.to(device)
                s2_mask_batch = s2_mask_batch.to(device)
                s1_mask_batch = s1_mask_batch.to(device)
    ...
    with torch.no_grad():
        for s2_batch, s1_batch, s2_mask_batch, s1_mask_batch, labels_batch in tqdm.tqdm(test_loader, desc="Test"):
            s2_batch = s2_batch.to(device)
            s1_batch = s1_batch.to(device)
            s2_mask_batch = s2_mask_batch.to(device)
            s1_mask_batch = s1_mask_batch.to(device)
