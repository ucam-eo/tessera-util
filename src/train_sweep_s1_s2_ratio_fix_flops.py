...
# You will need ptflops installed, e.g. "pip install ptflops"
...
CSV_PATH = "/mnt/e/Codes/btfm4rs/param_sweep.csv"
TOTAL_FLOPS_BUDGET = 1.0e15   # example total FLOPs budget
BATCH_SIZE = 512              # keep batch size constant across runs
HOST_NAME = "4090PC"          # used to label runs from a specific machine
...
# ---------------------------------------------------------------------
#   2.  Measure model FLOPs given hyperparameters
# ---------------------------------------------------------------------
def measure_model_flops(
    s2nhead, s2layer, s2dim_feedforward,
    s1nhead, s1layer, s1dim_feedforward,
    s2_band, s1_band,
    latent_dim=128,
    fusion_method="sum",
    sample_size_s2=20,
    ...
    """
    Measures approximate FLOPs for a single forward pass (batch_size=1).
    Then you can multiply by the real batch size and approx. 2 for backward.
    """
    ...
    s1_enc = TransformerEncoder(
        ...
        dropout=0.1,
        max_seq_len=sample_size_s1
    ).to(device)

    if fusion_method == 'concat':
        proj_in_dim = latent_dim * 2
    else:
        proj_in_dim = latent_dim

    projector = ProjectionHead(proj_in_dim, projector_hidden_dim, projector_out_dim).to(device)
    ...
    model = MultimodalBTModel(
        ...
        return_repr=True,
        latent_dim=latent_dim if fusion_method == 'transformer' else None
    ).to(device)
    ...
    with torch.no_grad():
        ...
        def wrapped_forward(*args, **kwargs):
            if kwargs:
                return original_forward(kwargs["s2_x"], kwargs["s1_x"])
            return original_forward(*args)
        model.forward = wrapped_forward
        macs, params = get_model_complexity_info(
            model,
            (1, sample_size_s2, s2_band),
            as_strings=False,
            print_per_layer_stat=False,
            verbose=False,
            input_constructor=lambda input_res: {
                "s2_x": torch.zeros((1, sample_size_s2, s2_band), device=device),
                "s1_x": torch.zeros((1, sample_size_s1, s1_band), device=device)
            }
        )
    return macs
...
# ----------------------------------------------------------------------
#   3.  The actual training / run function for wandb
# ----------------------------------------------------------------------
def train_and_evaluate():
    ...
    try:
        ...
        flops_per_forward = measure_model_flops(
            s2nhead, s2layer, s2dim_feedforward,
            s1nhead, s1layer, s1dim_feedforward,
            s2_band=12,
            s1_band=4,
        ...
        flops_per_step = flops_per_forward * BATCH_SIZE * 2.0
        max_steps = int(TOTAL_FLOPS_BUDGET // flops_per_step)
        if max_steps < 10:
            max_steps = 10

        wandb.config["computed_max_steps"] = max_steps
        logging.info(f"Row={row_idx}: FLOPs/step={flops_per_step:.2e}, max_steps={max_steps}")
                ...
                if step % val_interval == 0 and step > 0:
                    ...
                    if all(k in training_config for k in val_keys) and all(training_config[k] for k in val_keys):
                        ...
                        if val_acc > best_val_acc:
                            ...
                            ckpt_name = f"/mnt/e/Codes/btfm4rs/checkpoints/sweep_s1_s2_ratio/best_model_row{row_idx}_step{step}.pt"
...
# ----------------------------------------------------------------------
#   4.  The main function that sets up the Sweep
# ----------------------------------------------------------------------
def main():
    # 设置随机数种子
