...
TOTAL_FLOPS_BUDGET = 1.0e15        # 示例总 FLOPs 预算
...
# ---------------------------------------------------------------------
#   2.  根据一行超参数测量模型 FLOPs（单次前向，batch_size=1）
# ---------------------------------------------------------------------
def measure_model_flops(
    ...
    """
    测量单次前向传播（batch_size=1）的 FLOPs。
    然后你可以将其乘以真实的 batch_size，再乘以大约 2（考虑反向传播）得到每步 FLOPs。
    """
    # 1) 构建编码器（此处使用 CPU 测试）
    ...
    # 3) 使用 ptflops 测量。input_constructor 返回的字典键名需要与模型 forward 参数匹配
    with torch.no_grad():
        ...
        macs, params = get_model_complexity_info(
            model,
            (1, sample_size_s2, s2_band),   # s2_x 的 dummy 输入（含 batch 维度 1）
            as_strings=False,
    ...
    flops_for_batch1 = macs
    return flops_for_batch1
    ...
    try:
        ...
        training_config = {
            ...
            "rust_cmd": (
                ...
                "--data-root /home/zf281/rds/rds-sj514-data-WBrUDmBgqOo/s2_s1_global_project/sweep_s1_s2_ratio_data "
