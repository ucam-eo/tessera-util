...
class PastisPatchDataset(Dataset):
    def __init__(self, rep_files, target_files, transform=None, augment=False):
        """
        :param rep_files: representation .npy 文件列表，形状 (patch_size, patch_size, 64)
    ...
    def __getitem__(self, idx):
        ...
        # representation: (patch_size, patch_size, 64) -> (64, patch_size, patch_size)
...
class UNet(nn.Module):
    def __init__(self, in_channels=64, num_classes=20, features=[128, 256, 512], dropout=0.1):
        """
        :param in_channels: 输入通道数（这里为 64）
...
class DepthwiseUNet(nn.Module):
    """使用深度可分离卷积的UNet"""
    def __init__(self, in_channels=64, num_classes=20, 
...
class LightweightSegNet(nn.Module):
    """轻量级分割网络，参数量小于5M"""
    def __init__(self, input_channels=64, num_classes=20, dropout_rate=0.2, width_multiplier=0.25):
...
class EfficientLightSegNet(nn.Module):
    """高效轻量级分割网络，专为高维特征表示(64通道)设计"""
    def __init__(self, input_channels=64, num_classes=20, dropout_rate=0.1):
        ...
        self.dec1 = nn.Sequential(
            DepthwiseSeparableConv(128, 64),  # 128 = 64 (上采样) + 64 (跳跃连接)
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True)
        )
...
def train_model(model, train_loader, val_loader, device, num_classes, epochs=300, lr=3e-4, model_name="effective_segnet"):
    ...
    checkpoint_path = os.path.join(checkpoint_dir, f"best_pastis_patch_seg_{model_name}_efm_ckpt.pth")
...
def test_model(model, test_loader, device, num_classes, model_name="effective_segnet"):
    """测试模型并返回详细的评估指标"""
    # 加载最佳 checkpoint
    checkpoint_path = os.path.join("checkpoints", "downstream", f"best_pastis_patch_seg_{model_name}_efm_ckpt.pth")
...
def main():
    ...
    rep_root_dir = "/scratch/zf281/pangaea-bench/data/PASTIS-HD/AE_EMBEDDING/processed"
    ...
    if model_type == "unet":
        model = UNet(in_channels=64, num_classes=num_classes, dropout=0.1)
        model_name = "unet"
