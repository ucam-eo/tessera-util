...
fn process_tile(tile_dir: &Path, args: &Args) -> Result<Vec<SampleOut>> {
    ...
    let result_vec: Vec<SampleOut> = (0..valid_count).into_par_iter()
        .map(|i| {
            ...
            // 收集 Sentinel-2 中有效的时间索引（已经按时间顺序排列）
...
/// 关键改动：
/// 1. 每次采样时先从有效时间索引中随机选择 time_steps 个样本（允许重复），
/// 2. 将采样结果按原始时间（索引）排序，使得最终输出的时间顺序与原始数据一致。
fn sample_s2_pixel(
    s2_bands: &Array4<u16>,
    s2_doys: &Array1<u16>,
    valid_idx: &Vec<usize>,
    ...
    // 采样 time_steps 个有效索引（允许重复）
    let mut sampled_indices: Vec<usize> = (0..time_steps)
        .map(|_| valid_idx[rng.gen_range(0..n_valid)])
        .collect();
    // 按索引排序，保证时间顺序（原始的 valid_idx 已经是按时间顺序排列）
    sampled_indices.sort();

    for (t, &pick) in sampled_indices.iter().enumerate() {
        // 读取 10 个 band 并归一化
        let mut tmp = [0f32; 10];
        for b in 0..10 {
...
fn sample_s1_pixel(
    ...
    for arr in [&mut arr1, &mut arr2].iter_mut() {
        ...
        let mut sampled_items: Vec<(bool, usize, i16)> = (0..time_steps)
            ...
            .collect();
        // 按 doy 排序，保证时间顺序
        sampled_items.sort_by_key(|&(_is_asc, _idx, doy)| doy);

        for (t, &(is_asc, idx, doy_val)) in sampled_items.iter().enumerate() {
            let (raw_vv, raw_vh) = if is_asc {
                (asc_bands[(idx, yy, xx, 0)] as f32, asc_bands[(idx, yy, xx, 1)] as f32)
            } else {
                (desc_bands[(idx, yy, xx, 0)] as f32, desc_bands[(idx, yy, xx, 1)] as f32)
            };
            let vv_norm = (raw_vv - S1_BAND_MEAN[0]) / (S1_BAND_STD[0] + 1e-9);
            let vh_norm = (raw_vh - S1_BAND_MEAN[1]) / (S1_BAND_STD[1] + 1e-9);

            let dn = doy_val as f32 / 365.0;
            let sin_d = (2.0 * std::f32::consts::PI * dn).sin();
            let cos_d = (2.0 * std::f32::consts::PI * dn).cos();
